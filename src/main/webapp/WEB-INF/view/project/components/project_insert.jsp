        ...
        <section id="pro_detail_sec">
            <article id="customer_res">
                <div>
                    <label for="">
                        *프로젝트명
                        <input type="text" name="project_name" placeholder="프로젝트명">
                    </label>
                </div>
                <div id="pro_customer_area">
                    ...
                    <label for="f_cus_phone">
                        *고객사 번호
                        <input type="text" id="f_cus_phone" class="cp_num" name="f_cus_phone" maxlength="3" oninput="insert_numChk(this)">
                        <span>-</span>
                        <input type="text" id="m_cus_phone" class="cp_num" name="m_cus_phone" maxlength="4" oninput="insert_numChk(this)">
                        <span>-</span>
                        <input type="text" id="e_cus_phone" class="cp_num" name="e_cus_phone" maxlength="4" oninput="insert_numChk(this)">
                        <input type="hidden" name="main_num" value="" ">
                    </label>
                    <input type="hidden" name="customer_idx" value="" ">
            ...
            <article id="project_info">
                <div>
                    <label for="p_start_date">
                        시작일
                        <input type="date" name="p_start_date" min="0000-01-01" max="9999-12-31">
                    </label>
                </div>
                <div>
                    <label for="p_last_date">
                        종료일
                        <input type="date" name="p_last_date" min="0000-01-01" max="9999-12-31">
                    </label>
                </div>
                <div>
                    <label for="">
                        주소
                        <input type="text" id="sample6_postcode" name="p_zip_code" value="" placeholder="우편번호">
                        <input type="button" onclick="adress_search()" value="우편번호 찾기"><br>
                        <input type="text" id="sample6_address" name="p_main_adress" value="" placeholder="주소"><br>
                        <input type="text" id="sample6_detailAddress" name="p_detail_adress" value="" placeholder="상세주소">
                        <input type="text" id="sample6_extraAddress" name="p_extra_adress" value="" placeholder="참고항목">
                    </label>
                </div>
                <div>
                    <label for="rep_pm_name">
                        대표 PM
                        <input type="text" id="rep_pm_name" name="rep_pm_name">
                    </label>
                    <label for="rep_pm_phone">
                        대표 PM 번호
                        <input type="text" id="rep_pm_phone" class="rep_pm_phone" name="f_phone" maxlength="3" oninput="insert_numChk(this)">
                        <span>-</span>
                        <input type="text" id="rep_pm_phone" class="rep_pm_phone" name="m_phone" maxlength="4" oninput="insert_numChk(this)">
                        <span>-</span>
                        <input type="text" id="rep_pm_phone" class="rep_pm_phone" name="e_phone" maxlength="4" oninput="insert_numChk(this)">
                    </label>
                </div>
            </article>
            <article>
            ...
            <input type="button" value="프로젝트 등록" onclick="project_insert_go(this)">
            <input type="button" value="닫기" onclick="close_insert()">
        </section>
        <script>
            // 삽입 함수
            function project_insert_ajax(project_insert_data){
                $.ajax({
                    type: "post",
                    url: "project_insert_ajax",
                    contentType: "application/json",
                    data: JSON.stringify(project_insert_data),
                    dataType: "json",
            ...
            // 유효성 검사 및 데이터 담기
            function project_insert_go(section){
                let project_insert_data = {};
                ...
                let checkboxs = $("#pro_skill_insert").find('input[type="checkbox"]').filter(':checked');
                let selects = insert_section.find('select');
                let input_note = insert_section.find('textarea[name="note"]');
                let input_customer_idx = insert_section.find('input[name="customer_idx"]');
                
            
                let input_project_name = insert_section.find('input[name="project_name"]');
                let input_customer_name = insert_section.find('input[name="customer_name"]');
                let input_f_cus_phone = insert_section.find('input[name="f_cus_phone"]');
                let input_m_cus_phone = insert_section.find('input[name="m_cus_phone"]');
                let input_e_cus_phone = insert_section.find('input[name="e_cus_phone"]');
                ...
                $.each(input_texts, function (idx, k) { 
                    if (k.name) {
                        project_insert_data[k.name] = k.value;
                    }
                });
                ...
                project_insert_data["skill_list"] = chk_val;
                project_insert_data["customer_idx"] = input_customer_idx.val();
                project_insert_data["note"] = input_note.val();
                
                project_insert_ajax(project_insert_data)
            }
